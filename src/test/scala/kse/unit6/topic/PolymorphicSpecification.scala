...
object CompositionSpecification extends Properties("Composition"):
  ...
  property("Polymorphic composition of function should be correctly evaluated") = forAll:
    (f: Int => String, g: Boolean => Int, x: Boolean) => `f(g(x))`(f)(g)(x) == (f compose g)(x)
...
object YCombinatorSpecification extends Properties("Y-combinator"):
  ...
  property("Polymorphic Y-combinator should allow to evaluate factorial recursively") = forAll: (value: BigInt) =>
    Y(almostFactorialFunction)(value) == (0 to value.toInt).fold(1)((acc, value) =>
      if value == 0 then acc else acc * value
    )
