...
//    ???
  ...
  property("An intersections of two given sets should contain the same elements from both sets") = forAll:
    (left: Set[Numeral], right: Set[Numeral]) =>
      (left ∩ right).forAll(element => left.contains(element) && right.contains(element)) &&
        left.forAll(element => !right.contains(element) || (left ∩ right).contains(element))
...
//    ???
  ...
  property("Union should be distributive over intersection") = forAll:
    (a: Set[Numeral], b: Set[Numeral], c: Set[Numeral]) => (a ∪ (b ∩ c)) == ((a ∪ b) ∩ (a ∪ c))

  property("Intersection should be distributive over union") = forAll:
    (a: Set[Numeral], b: Set[Numeral], c: Set[Numeral]) => (a ∩ (b ∪ c)) == ((a ∩ b) ∪ (a ∩ c))
